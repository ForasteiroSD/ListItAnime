...
function Animes() {
  ...
  const searchAnimes = async (e) => {
    const interval = setInterval(async () => {
      ...
      if(buffer === '') getRecomendations();
        ...
        const response = await Axios.get('http://127.0.0.1:5000/search?name=' + buffer);
        const animes = response.data;
        setAnimes(animes);
      }
    }, 1000);
  ...
  if(animes) {
    return (
      <>
        <input type="text" onChange={handleSearchChange} onFocus={searchAnimes} onBlur={cancelSearchAnimes}/>
